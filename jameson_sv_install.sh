...
#VTK args
VTK_CMAKE_ARGS="-D BUILD_SHARED_LIBS=OFF -D BUILD_TEST=OFF -D VTK_Group_Tk=ON"
VTK_CMAKE_ARGS+="-D TCL_INCLUDE_PATH=${TCL_INCLUDE}"
VTK_CMAKE_ARGS+="-D TCL_LIBRARY=${TCL_LIB}"
VTK_CMAKE_ARGS+="-D TCL_TCLSH=${TCL_BIN}"
VTK_CMAKE_ARGS+="-D TK_INCLUDE_PTH=${TK_INCLUDE}"
VTK_CMAKE_ARGS+="-D TK_LIBRARY=${TK_LIB}"
VTK_CMAKE_ARGS+="-D CMAKE_C_FLAGS=-DGLX_GLXEXT_LEGACY -D CMAKE_CXX_FLAGS=-DGLX_GLXEXT_LEGACY"
VTK_CMAKE_ARGS+="-D VTK_WRAP_TCL=ON -D Module_vtkWrappingTcl=ON -D VTK_USE_TK=ON"

VTK_DIR="${1}VTK6/build"

#ITK Args
ITK_CMAKE_ARGS="-D BUILD_SHARED_LIBS=OFF"
ITK_CMAKE_ARGS+="-D BUILD_TESTING=OFF"
ITK_CMAKE_ARGS+="-D Module_ITKVtkGlue=ON"
ITK_CMAKE_ARGS+="-D ITK_Review=ON"
ITK_CMAKE_ARGS+="-D VTK_DIR=${VTK_DIR}"

ITK_DIR="${1}ITK4.9/build"

#SV Args
SV_CMAKE_ARGS="-D SimVascular_SUPERBUILD=OFF "
SV_CMAKE_ARGS+="-D SimVascular_USE_SYSTEM_ITK=ON -D SimVascular_USE_SYSTEM_VTK=ON "
SV_CMAKE_ARGS+="-D ITK_DIR=${ITK_DIR} -D VTK_DIR=${VTK_DIR} "
SV_CMAKE_ARGS+="-D TCL_INCLUDE_PATH=${TCL_INCLUDE} -D TCL_LIBRARY=${TCL_LIB} "
SV_CMAKE_ARGS+="-D TCL_TCLSH=${TCL_BIN} "
SV_CMAKE_ARGS+="-D TK_INCLUDE_PATH=${TK_INCLUDE} -D TK_WISH=${TK_WISH}"
